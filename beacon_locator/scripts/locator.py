...
class reader:
	...
	def rgbCallback(self, img):
		...
		for rect1, rect2 in itertools.permutations(rects, 2):
			...
			if rect1[6] < rect2[6] and (((rect1[1] >= rect2[1] and rect1[1] <= rect2[3]) or (rect1[3] >= rect2[1] and rect1[3] <= rect2[3])) or ((rect2[1] >= rect1[1] and rect2[1] <= rect1[3]) or (rect2[3] >= rect1[1] and rect2[3] <= rect1[3]))) and rect1[4] - rect2[2] < 20:
				for b in self.beacons:
					if b["top"] == rect1[0] and b["bottom"] == rect2[0]:
						print "Beacon %d: %s / %s" % (b["id"], rect1[0], rect2[0])
